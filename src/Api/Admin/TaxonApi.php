...
final class TaxonApi implements TaxonApiInterface
{
    ...
    {
        Assert::string($code);
        return $this->resourceClient->getResource('api/v2/admin/taxons/%s', [$code]);
    }

    public function upsert($code, array $data = []): int
    {
        Assert::string($code);
        return $this->resourceClient->upsertResource('api/v2/admin/taxons/%s', [$code]);
    }
    ...
    public function listPerPage(
        ...
        $data = $this->resourceClient->getResources('api/v2/admin/taxons', [], $limit, $queryParameters, $filterBuilder, $sortBuilder);
