...
final class TaxCategoryApi implements TaxCategoryApiInterface
{
    ...
    {
        Assert::string($code);
        return $this->resourceClient->getResource('api/v2/admin/tax-categories/%s', [$code]);
    }

    public function upsert($code, array $data = []): int
    {
        Assert::string($code);
        return $this->resourceClient->upsertResource('api/v2/admin/tax-categories/%s', [$code]);
    }
    ...
    public function listPerPage(
        ...
        $data = $this->resourceClient->getResources('api/v2/admin/tax-categories', [], $limit, $queryParameters, $filterBuilder, $sortBuilder);
    ...
    {
        Assert::string($code);
        return $this->resourceClient->deleteResource('api/v2/admin/tax-categories/%s', [$code]);
    }
}
