...
final class ProductApi implements ProductApiInterface
{
    ...
    {
        Assert::string($code);
        return $this->resourceClient->getResource('api/v2/admin/products/%s', [$code]);
    }

    public function upsert($code, array $data = []): int
    {
        Assert::string($code);
        return $this->resourceClient->upsertResource('api/v2/admin/products/%s', [$code]);
    }
    ...
    public function listPerPage(
        ...
        $data = $this->resourceClient->getResources('api/v2/admin/products', [], $limit, $queryParameters, $filterBuilder, $sortBuilder);
