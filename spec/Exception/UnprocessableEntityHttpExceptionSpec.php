...
class UnprocessableEntityHttpExceptionSpec extends ObjectBehavior
{
    ...
    public function it_exposes_the_response_errors($response, StreamInterface $body)
    {
    ...
    public function it_returns_an_empty_array_when_the_response_has_no_errors($response, StreamInterface $body)
    {
        $response->getStatusCode()->willReturn(422);
        $response->getBody()->willReturn($body);
        $body->rewind()->shouldBeCalled();
    ...
    public function it_returns_an_empty_array_when_the_body_is_not_a_valid_json($response, StreamInterface $body)
    {
        $response->getStatusCode()->willReturn(422);
        $response->getBody()->willReturn($body);
